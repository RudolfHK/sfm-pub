...
class SfMVisualizer:
    ...
    def __init__(
        ...
        base_name = base_out.name or "sfm_visualization"
        self._out = base_out.parent / f"{base_name}_{timestamp}"
    ...
    def on_camera_registered(
        ...
        try:
            self._render_registration_step(
                step, image_id, cameras_snap, pts_snap, n_new_pts, is_seed
            )
        except Exception as exc:
    ...
    def _render_keypoint_overlay(self, idx: int) -> None:
        ...
        if len(kps) > 0:
            ...
            ax.scatter(kps[:, 0], kps[:, 1], c=colors, s=12, linewidths=0, alpha=0.7)
    ...
    def _render_feature_statistics(self) -> None:
        ...
        axes[0].axvline(
            np.mean(counts), color="red", ls="--", label=f"mean={np.mean(counts):.0f}"
        )
        ...
        axes[1].barh(range(top_n), [counts[i] for i in sorted_idx], color=bar_colors)
        ...
        fig.tight_layout()
        self._save_fig(
            fig, self._out / "01_features" / f"feature_statistics.{self._fmt}"
        )
    ...
    def _render_connectivity_graph(self) -> None:
        ...
        nx.draw_networkx_nodes(
            G, pos, nodelist=connected, node_color="steelblue", node_size=500, ax=ax
        )
        nx.draw_networkx_nodes(
            G, pos, nodelist=isolated, node_color="red", node_size=500, ax=ax
        )
        nx.draw_networkx_edges(
            G,
            pos,
            width=widths,
            edge_color=n_inliers,
            edge_cmap=self._cm.YlOrRd,
            ax=ax,
            alpha=0.8,
        )
        ...
        fig.tight_layout()
        self._save_fig(
            fig, self._out / "02_matching" / f"connectivity_graph.{self._fmt}"
        )
    ...
    def _render_registration_step(
        ...
        for ci, cam_idx in enumerate(cam_list):
            ...
            ax1.scatter(
                ...
                c=[color],
                zorder=zorder,
                edgecolors="yellow" if is_new else "none",
                linewidths=1.5,
            )
            ax1.annotate(
                str(cam_idx),
    ...
    def _render_ba_convergence(self) -> None:
        ...
        xs = np.array(rounds)
        ax2.bar(
            xs - width / 2, n_pts_b, width, label="Before BA", color="tomato", alpha=0.7
        )
        ax2.bar(
            xs + width / 2,
            n_pts_a,
        ...
        self._save_fig(
            ...
            self._out
            / "03_reconstruction"
            / f"bundle_adjustment_convergence.{self._fmt}",
        )
    ...
    def _render_point_lifecycle(self) -> None:
        ...
        ax.axvline(
            obs_count.mean(), color="red", ls="--", label=f"mean={obs_count.mean():.1f}"
        )
        ax.legend(fontsize=9)
        ...
        fig.tight_layout()
        self._save_fig(
            fig, self._out / "03_reconstruction" / f"point_lifecycle.{self._fmt}"
        )
    ...
    def _render_camera_poses(self) -> None:
        ...
        if len(centers) > 1:
            ...
            ax.plot(traj[:, 0], traj[:, 1], traj[:, 2], "w--", lw=0.8, alpha=0.4)
        ...
        self._save_fig(
            fig, self._out / "03_reconstruction" / f"camera_poses_final.{self._fmt}"
        )
    ...
    def _render_reprojection_errors(self, img_idx: int) -> None:
        ...
        ax.axis("off")
        out = (
            self._out / "03_reconstruction" / f"reprojection_errors_{stem}.{self._fmt}"
        )
        self._save_fig(fig, out)
    ...
    def _render_pointcloud_views(self) -> None:
        ...
        for i, (title, elev, azim) in enumerate(views, 1):
            ...
            ax.scatter(pts[:, 0], pts[:, 1], pts[:, 2], c=c_norm, s=0.5, alpha=0.7)
        ...
        fig.tight_layout()
        self._save_fig(
            fig, self._out / "04_pointcloud" / f"pointcloud_6views.{self._fmt}"
        )
    ...
    def _render_summary_dashboard(self, stats: dict) -> None:
        ...
        if len(self._pts3d) > 0:
            pts = self._pts3d
            c_norm = (
                self._colors.astype(np.float32) / 255.0
                if self._colors is not None and len(self._colors) == len(pts)
                else "steelblue"
            )
            if len(pts) > 10_000:
    ...
    def _save_reconstruction_video(self) -> None:
        try:
            ...
            logger.warning(
                "[VIZ] imageio not installed — skipping reconstruction video"
            )
            return
    ...
    def _save_fig(self, fig, path: Path) -> None:
        ...
        fig.savefig(
            str(path), dpi=self._dpi, bbox_inches="tight", facecolor=fig.get_facecolor()
        )
        self._plt.close(fig)
        self._n_saved += 1
    ...
    def _sample_image_indices(self, n: int, registered: Optional[dict] = None) -> list:
        """Return up to n random image indices, preferring registered cameras."""
        if not self._features:
            return []
        pool = list(registered.keys()) if registered else list(self._features.keys())
        k = min(n, len(pool))
